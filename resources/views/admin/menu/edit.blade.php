...
<div class="container mx-auto px-6 py-12 max-w-3xl">
    ...
    <!-- Form -->
    <form action="{{ route('admin.menu.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-8">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea name="description" 
                      ...
                      class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-orange-500">{{ old('description', $product->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div> -->
        ...
        <div class="mb-6">
            ...
            <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="w-32 h-32 rounded-lg object-cover">
        </div>
        @endif
